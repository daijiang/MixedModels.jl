...
"""
    fixef!{T}(v::Vector{T}, m::LinearMixedModel{T}, force=false)

Overwrite `v` with the fixed-effects coefficients of model `m`.

If `force` is `true`, the computation is always done, otherwise an `ArgumentError`
is thrown if `!isfit(m)`.
"""
function fixef!{T}(v::AbstractVector{T}, m::LinearMixedModel{T}, force=false)
    if !(force || isfit(m))
